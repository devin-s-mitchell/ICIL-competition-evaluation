...
- (feat): `icilval store mirror --prune` makes the dataset exactly the local store in one commit (upload every file, delete every stale path), for a store rebuilt after a schema change.
